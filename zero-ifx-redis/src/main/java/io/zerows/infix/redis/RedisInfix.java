...
public class RedisInfix implements Infix {
    ...
    private static JsonObject initSync(final String name, final JsonObject options) {
        if (!CLIENTS_SYNC.containsKey(name)) {
            ...
            if (!options.containsKey(YmlCore.redis.ENDPOINT)) {
                options.put(YmlCore.redis.ENDPOINT, "redis://" + host + ":" + port);
            }
            final Jedis client = new Jedis(host, port);
            // Auth
            final String password = options.getString(YmlCore.redis.PASSWORD);
            if (Ut.isNotNil(password)) {
                final String username = options.getString(YmlCore.redis.PASSWORD);
