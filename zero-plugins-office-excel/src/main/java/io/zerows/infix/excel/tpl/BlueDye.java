package io.zerows.infix.excel.tpl;
 ...
 */
class BlueDye {
    private static final Cc<Integer, BlueDye> CC_PICKER = Cc.open();
    /*
     * Fix Issue:
    ...
    void onEmpty(final Cell cell) {
        final DyeCell dyeCell = Cc.pool(this.stylePool, "EMPTY",
            () -> DyeCell.create(this.workbook)
                .align(HorizontalAlignment.CENTER)
                .border(BorderStyle.THIN));
        cell.setCellStyle(dyeCell.build());
    ...
    private DyeCell onDataValue(final String key) {
        return Cc.pool(this.stylePool, key,
            () -> DyeCell.create(this.workbook)
                .border(BorderStyle.THIN)
