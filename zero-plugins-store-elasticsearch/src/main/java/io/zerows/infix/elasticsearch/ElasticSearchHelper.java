package io.zerows.infix.elasticsearch;
...
import io.zerows.infix.elasticsearch.exception._404ConfigurationMissingExceptionn;
    ...
    RestHighLevelClient getClient(final JsonObject options) {
        ...
        credentialsProvider.setCredentials(AuthScope.ANY,
            new UsernamePasswordCredentials(
                options.getString(YmlCore.elasticsearch.USERNAME),
                options.getString(YmlCore.elasticsearch.PASSWORD)
            )
        );

        return new RestHighLevelClient(
            RestClient.builder(new HttpHost(
                    options.getString(YmlCore.elasticsearch.HOSTNAME),
                    options.getInteger(YmlCore.elasticsearch.PORT),
                    options.getString(YmlCore.elasticsearch.SCHEME)
                ))
