package io.zerows.infix.neo4j.sync;

import io.vertx.core.Future;
import io.vertx.core.json.JsonArray;
import io.vertx.core.json.JsonObject;
import io.vertx.up.util.Ut;
import io.zerows.infix.neo4j.AbstractN4JSession;
import io.zerows.infix.neo4j.refine.N4J;
...
public class N4JSession extends AbstractN4JSession {
    ...
    private transient io.zerows.infix.neo4j.sync.N4JOp nodeOp;
    private transient io.zerows.infix.neo4j.sync.N4JOp edgeOp;

    public N4JSession(final String graph) {
        super(graph);
    }

    private io.zerows.infix.neo4j.sync.N4JOp nodeOp() {
        if (Objects.isNull(this.nodeOp)) {
            this.nodeOp = new io.zerows.infix.neo4j.sync.N4JOpNode(this.graph, this.driver);
        }
        return this.nodeOp;
    }

    private io.zerows.infix.neo4j.sync.N4JOp edgeOp() {
        if (Objects.isNull(this.edgeOp)) {
            this.edgeOp = new io.zerows.infix.neo4j.sync.N4JOpEdge(this.graph, this.driver);
        }
        return this.edgeOp;
    }
